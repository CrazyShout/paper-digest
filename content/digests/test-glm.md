...
### 协同轨迹预测
**CoPAD** 提出轻量级协同轨迹预测框架，使用匈牙利算法和卡尔曼滤波做多源轨迹融合，在 DAIR-V2X-Seq 数据集上达到 SOTA。
...
### 应用场景二：车路协同系统部署
Consensus ADMM、CoPAD 和 Evaluating Roadside Perception 共同构成了车路协同系统的技术栈。路侧感知评估指导部署，协同规划和预测在部署后运行，形成闭环优化。
