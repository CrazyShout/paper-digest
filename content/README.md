...
正文请完整复制 [深读版固定模板](templates/paper-report-template.md) 的八章骨架，按其中的写作任务完成分析。可对照 [DA-WAM 样例](papers/da-wam-decision-aligned-world-model.md) 检查公式解释、实验表格、相关工作差异和最小验证计划。此处只保留 frontmatter 示例，避免维护两份不同的正文模板。
